...
void DJI::onboardSDK::API::appHandler(Header *header)
{
    ...
    if(header->is_ack == 1)
    {
        if(header->session_id > 1 && header->session_id < 32)
        {
            ...
            {
                driver->lockMemory();
                p2header = (Header*)CMDSessionTab[header->session_id].mmu->pmem;
                        ...
                        p2header->sequence_number == header->sequence_number)
                {
                    ...
                    Call_APP_Func = CMDSessionTab[header->session_id].ack_callback;
                    freeSession(&CMDSessionTab[header->session_id]);
                    driver->freeMemory();
                    if(Call_APP_Func)
                    {
                        Call_APP_Func(this,header);
                    }
                }
                else
                {
    ...
    else
    {
        ...
        case 1:
        default:
            if(ACKSessionTab[header->session_id - 1].session_status == ACK_SESSION_PROCESS)
            {
                API_DEBUG("%s,This session is waiting for App ack:"
                       "session id=%d,seq_num=%d\n",__func__,
                       header->session_id,header->sequence_number);
            }
            else if(ACKSessionTab[header->session_id - 1].session_status == ACK_SESSION_IDLE)
            {
                if(header->session_id > 1)
                {
                    ACKSessionTab[header->session_id - 1].session_status = ACK_SESSION_PROCESS;
                }

                recvReqData(header);
            }
            else if(ACKSessionTab[header->session_id - 1].session_status == ACK_SESSION_USING)
            {
                driver->lockMemory();
                p2header = (Header *)ACKSessionTab[header->session_id - 1].mmu->pmem;
                if(p2header->sequence_number == header->sequence_number)
                {
                    API_DEBUG("%s:repeat ACK to remote,session id=%d,seq_num=%d\n",
                           __func__,header->session_id,header->sequence_number);
                    sendData(ACKSessionTab[header->session_id - 1].mmu->pmem);
                    driver->freeMemory();
                }
                else
                {
                    API_DEBUG("%s:same session,but new seq_num pkg,session id=%d,"
    ...
    for(i = 1 ; i < SESSION_TABLE_NUM ; i ++)
    {
        if(CMDSessionTab[i].usage_flag == 1)
        {
                    ...
                    > CMDSessionTab[i].ack_timeout)
            {
                ...
                {
                    if(CMDSessionTab[i].sent_time >= CMDSessionTab[i].retry_send_time )
                    {
                        API_DEBUG("%s:Free ssion %d\n",__func__,CMDSessionTab[i].session_id);
...
static unsigned short Pro_Calc_Length_Lambda(unsigned short size, unsigned short encrypt_flag)
{
    unsigned short len;
    if(encrypt_flag)
    {
...
int DJI::onboardSDK::API::ackInterface(Ack *parameter)
{
    ...
    else if(parameter->session_id > 0 && parameter->session_id < 32)
    {
        ...
        ack_session = allocACK(parameter->session_id,
                               Pro_Calc_Length_Lambda(parameter->length,parameter->need_encrypt));
        ...
        ret = encrypt(ack_session->mmu->pmem,parameter->buf,
...
int DJI::onboardSDK::API::sendInterface(Command *parameter)
{
    ...
    case 0:
        driver->lockMemory();
        cmd_session = allocSession(CMD_SESSION_0,Pro_Calc_Length_Lambda(parameter->length,parameter->need_encrypt));
        ...
        ret = encrypt(cmd_session->mmu->pmem,parameter->buf,parameter->length,
                      0,parameter->need_encrypt,cmd_session->session_id,seq_num);
        if(ret == 0)
        {
            API_ERROR("%s:%d:encrypt ERROR\n",__func__,__LINE__);
    ...
    case 1:
        driver->lockMemory();
        cmd_session = allocSession(CMD_SESSION_1,Pro_Calc_Length_Lambda(parameter->length,parameter->need_encrypt));
        if(cmd_session == (CMDSession *)NULL)
        {
            driver->freeMemory();
        ...
        API_DEBUG("%s,sending session %d\n",__func__,cmd_session->session_id);
        sendData(cmd_session->mmu->pmem);
        driver->freeMemory();
        break;
    case 2:
        driver->lockMemory();
        cmd_session = allocSession(CMD_SESSION_AUTO,Pro_Calc_Length_Lambda(parameter->length,parameter->need_encrypt));
        if(cmd_session == (CMDSession *)NULL)
        {
            driver->freeMemory();
            API_ERROR("%s:%d:ERROR,there is not enough memory\n",__func__,__LINE__);
        ...
        break;
    default:
        API_ERROR("Wrong mode:%d,%s,%d",parameter->session_mode,__func__,__LINE__);
        break;
    }
    return 0;
}
