...
void App_Recv_Req_Data(ProHeader *header)
{
    ...
    //printf("Recv request,sequence_number=%d,session_id=%d,len=%d\n", recv_req_id.sequence_number, recv_req_id.session_id, recv_data.len);
    //printf("Recv request,sequence_number=%d,session_id=%d,len=%d\n", header->sequence_number, header->session_id, header->length);
    memcpy((uint8_t *)&recv_data.cmd_set,(uint8_t *)&header->magic, recv_data.len);

    //printf("%s:Recv request,dev_id=%d,cmd_id=%d\n",
    //      __func__,recv_data.dev_id,recv_data.cmd_id);
...
void App_Send_Ack(req_id_t req_id, uint8_t *ack, int len)
{
    ...
    param.length = len;//sizeof(Pro_Encode_ACK);
