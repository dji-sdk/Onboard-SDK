                                        ...
                                        CommandResult user_notice_entrance)
{
    if (len > 100)
    {
        API_ERROR("%s: Too much data to send", __func__);
        return;
    }
                                        ...
                                        Header *header)
{
    ...
    {
        API_ERROR(
            "%s,line %d:ERROR,ACK is exception,seesion id %d,sequence %d\n",
...
void DJI::onboardSDK::API::setControlCallback(API *This, Header *header)
{
    ...
    {
        case 0x0000:
            API_STATUS("%s,line %d, obtain control failed, Conditions did not "
                       "satisfied",
                       __func__, __LINE__);
            break;
        case 0x0001:
