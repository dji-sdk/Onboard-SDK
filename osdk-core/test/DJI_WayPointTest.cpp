...
void
DJI_WayPointTest::set_waypoint_defaults(WayPointData* wp)
{
  wp->damping         = 0;
  wp->yaw             = 0;
  wp->gimbalPitch     = 0;
...
void
DJI_WayPointTest::set_waypoint_init_defaults(WayPointInitData* fdata)
{
...
void
DJI_WayPointTest::run_init_provide_test(int init_count, int prov_count)
{
...
void
DJI_WayPointTest::generate_waypoints(WayPointData* start_data,
...
WayPointData
DJI_WayPointTest::create_start_waypoint(BroadcastData* prior_data,
                                        float32_t      start_alt)
{
  WayPointData wp;
  this->set_waypoint_defaults(&wp);
...
void
DJI_WayPointTest::upload_waypoints(float64_t increment, float32_t start_alt,
                                   int wp_count)
{
  ...
  for (std::vector<WayPointData>::iterator wp = wp_list.begin();
       wp != wp_list.end(); ++wp)
  {
