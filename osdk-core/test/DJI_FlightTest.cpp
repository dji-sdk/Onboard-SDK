...
void
DJI_FlightTest::doTask(Flight::TASK task)
{
...
DJI_FlightTest::processTask()
{
  ...
  if (api->getFwVersion() < MAKE_VERSION(3, 2, 0, 0))
  {
    RESULT_STATUS = (task == Flight::TASK_TAKEOFF)
                      ? Flight::STATUS_SKY_STANDBY_M100ONLY
                      : Flight::STATUS_GROUND_STANDBY;
  }
  else
  {
    RESULT_STATUS = (task == Flight::TASK_TAKEOFF) ? Flight::STATUS_SKY_STANDBY
                                                   : Flight::STATUS_MOTOR_OFF;
  }
...
float32_t
DJI_FlightTest::waitToSyncHeight()
{
...
void
DJI_FlightTest::takeOffWithRetry()
{
  int result;
  int retry = 1;
  ...
  if (result == 0)
    ...
    ASSERT_PRED_FORMAT2(::testing::FloatLE, waitToSyncHeight(),
                        TAKE_OFF_MAX_HOVER_HEIGHT);
  else
    FAIL() << "Failed Task: TakeOFF";
}

float32_t
DJI_FlightTest::getHeight()
{
...
void
DJI_FlightTest::land(Flight::TASK task)
{
  doTask(task);
  if (processTask() == 0)
  {
...
//! @note last Test case is Wp still in air, need land first
