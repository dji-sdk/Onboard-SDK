...
#include "mop.h"
#include "core/mop/mop_entry_osdk.h"
#endif
#include "osdk_command_instance.h"
...
const static T_RouteKey routeTable[] = {
    ...
    {GEN_ADDR_INDEX_ONLY(ADDR_V1_COMMAND_INDEX), ADDR_INDEX_ONLY_MASK,
     USB_ACM_CHANNEL_ID},
    {GEN_ADDR_INDEX_ONLY(ADDR_BIGDATA_LIVEVIEW_INDEX), ADDR_INDEX_ONLY_MASK,
...
E_OsdkStat OsdkCommand_Init(T_CmdHandle *cmdHandle, const T_CmdInitConf *conf) {
  ...
  if (OsdkOsal_MutexCreate(&cmdHandle->waitAckItemMutex) != OSDK_STAT_OK) {
    OSDK_LOG_ERROR(MODULE_NAME_COMMAND, "waitAckItemMutex create error");
    goto wait_mutex_create_err;
  }

  if (OsdkOsal_MutexCreate(&cmdHandle->recvCmdHandleListMutex) != OSDK_STAT_OK) {
    OSDK_LOG_ERROR(MODULE_NAME_COMMAND, "recvCmdHandleListMutex create error");
    goto cmd_handle_mutex_create_err;
  }
...
E_OsdkStat OsdkCommand_CheckLiveViewChannel() {
  E_OsdkStat osdkStat;
  T_ChannelItem *channelItem;
...
#if defined(__linux__)
E_OsdkStat OsdkCommand_CreateMopTask() {
  if(mop_osdk_init() != MOP_SUCCESS)
    return OSDK_STAT_ERR;
  return OSDK_STAT_OK;
}

E_OsdkStat OsdkCommand_DestroyMopTask() {
  if(mop_osdk_deinit() != MOP_SUCCESS)
    return OSDK_STAT_ERR;
  return OSDK_STAT_OK;
}
#endif
  ...
  for (index = 0; index < PROT_MAX_WAIT_ACK_LIST; index++) {
    if (!cmdHandle->waitAckItem[index].isValid) {
      if (OsdkOsal_GetTimeMs(&cmdHandle->waitAckItem[index].sendTimeStamp) !=
        ...
        OsdkOsal_MutexUnlock(cmdHandle->waitAckItemMutex);
        goto err;
      }
...
#endif
          /*! this timeOutCmdInfo support callback getting cmdset and cmdid */
          T_CmdInfo timeOutCmdInfo = cmdHandle->waitAckItem[index].sendInfo;
          timeOutCmdInfo.dataLen = 0;
          timeOutCmdInfo.packetType = OSDK_COMMAND_PACKET_TYPE_ACK;
          timeOutCmdInfo.sender = cmdHandle->waitAckItem[index].sendInfo.receiver;
          timeOutCmdInfo.receiver = cmdHandle->waitAckItem[index].sendInfo.sender;
          cmdHandle->waitAckItem[index].callback(
              &timeOutCmdInfo, NULL, cmdHandle->waitAckItem[index].userData,
