 ...
 *
 *  @brief
 *  Implement send/read, app handling and data link layer for Core API of DJI
 * onboardSDK library
 *
...
void
CoreAPI::sendData(unsigned char* buf)
{
  size_t  ans;
...
void
CoreAPI::allocateACK(Header* protocolHeader)
{
...
void
CoreAPI::notifyCaller(Header* protocolHeader)
{
...
void
CoreAPI::notifyNonBlockingCaller(Header* protocolHeader)
{
...
void
CoreAPI::callbackPoll(CoreAPI* api)
{
  serialDevice->lockNonBlockCBAck();
...
void
CoreAPI::setActivation(bool isActivated)
{
...
void
DJI::onboardSDK::CoreAPI::setACKFrameStatus(uint32_t usageFlag)
{
  ackFrameStatus = usageFlag;
}

uint32_t
DJI::onboardSDK::CoreAPI::getACKFrameStatus()
{
  return ackFrameStatus;
}

void
CoreAPI::setSyncFreq(uint32_t freqInHz)
{
  send(0, 1, SET_SYNC, CODE_SYNC_BROADCAST, &freqInHz, sizeof(freqInHz));
}

unsigned short
calculateLength(unsigned short size, unsigned short encrypt_flag)
{
  unsigned short len;
  if (encrypt_flag)
...
int
CoreAPI::ackInterface(Ack* parameter)
{
...
int
CoreAPI::sendInterface(Command* parameter)
{
  unsigned short ret        = 0;
