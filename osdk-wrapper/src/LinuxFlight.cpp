...
{
  void (*pointerArmCallback)(CoreAPI*, Header*, DJI::UserData);
  pointerArmCallback = armCallback;
  flight->setArm(true,pointerArmCallback,NULL);
}

void disArmCallback(CoreAPI* api, Header *protHeader, DJI::UserData data) {
  uint16_t simpAck = api->missionACKUnion.simpleACK;
  printf("Disarm ACK is %d \n", simpAck);
...
//! Disarm the aircraft (Non-Blocking API call) and return status as well as ack
void disArmNonBlocking(Flight* flight)
{
  void (*pointerDisArmCallback)(CoreAPI*, Header*, DJI::UserData);
  pointerDisArmCallback = disArmCallback;
  flight->setArm(false,pointerDisArmCallback,NULL);
}
...
//! Takeoff (Non-Blocking API call) and return status as well as ack. The ack is only for starting takeoff
void takeoffNonBlocking(Flight* flight)
{
  void (*pointerTakeoffCallback)(CoreAPI*, Header*, DJI::UserData);
  pointerTakeoffCallback = takeoffCallback;
  flight->task(Flight::TASK_TAKEOFF,pointerTakeoffCallback,NULL );
}
...
{
  //Call landing task
  void (*pointerLandingCallback)(CoreAPI*, Header*, DJI::UserData);
  pointerLandingCallback = landingCallback;
  flight->task(Flight::TASK_LANDING,pointerLandingCallback,NULL );
}