...
### 控制模式
模式标志字节代表不同模式的配置。因为多旋翼的结构特点，飞行控制的时候，要把控制信息分解成三部分，竖直、水平和偏航，每个部分都有几种选择。
...
<tr>
  <td>HORI_VEL</td>
  <td>水平方向控制的是飞行器速度，根据坐标系确定水平分量</b></td>
</tr>
...
*\*HORI_ATTI_TILT_ANG模式控制量如下图，DJI飞控采用水平面直接进行整个平面旋转。其中平面旋转角度为Θ,旋转方向与x轴或roll轴方向角度为γ。输入参量Θx=Θ\*cos(γ),Θy=Θ\*sin(γ)。*  
<div align="center">
...
### 模式标志字节
竖直、水平和偏航的姿态控制通过模式标志字节进行组合。模式标志字节的定义如下。
<table>
<tr>
...
*\*模式标识位低3位决定水平方向及偏航坐标系，部分模式下可根据开发者需要决定使用哪种坐标系，这里xx表示水平方向坐标系的控制位，y表示偏航坐标系的控制位，具体控制关系请参阅模式标志字节说明。*
