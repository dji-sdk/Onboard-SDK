...
---
## 本文档撰写目的
在本文档中，我们着重介绍了在开放协议中，底层指令码流的格式及配套命令本身。为了能与N1飞行控制单元进行有效通讯，工作在本层的开发者需要在程序中构建稳定且以推荐时间间隔的指令码流，通过串口（UART）传输到N1飞行控制单元，经由N1飞行控制单元对目标飞行器进行一系列操作。
